...
| [Pricing & Cost Calculator](./pricing-cost-calculator.md) | Monetization | Built | `/console/pricing` (call-mix builder, plan comparison, volume-discount progress, overage, 12-month forecast), pricing SSOT `lib/pricing.ts` consumed by `/console/billing` and the gateway |
...
| [Lifecycle Nudges & Journey Orchestration](./lifecycle-nudges-plan.md) | Growth & Lifecycle | Built (prototype) | nudge surfaces console-wide (banners/modals/celebrations/toasts + orchestrator + watcher in the layout), `/console/journey` cockpit, lead scoring + sales signals, re-engagement email/webhook ledger, `lib/lifecycle.ts`/`lib/nudges.ts`/`lib/nudge-delivery.ts` SSOTs; pre-auth TOFU on `/` + `/signup` |
