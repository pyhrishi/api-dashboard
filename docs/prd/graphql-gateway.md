...
## 6. API & Gateway
- **`POST /api/graphql`** (`app/api/graphql/route.ts`) — middleware covers only `/api/v1`, so the route does its own **auth** (bearer, any well-formed `sk_test_`/`sk_live_`, billing lazily provisions), **billing** (`deductCredits(key, cost)`, `402` on insufficient), and **live-key governance mirroring REST exactly**: it derives the compliance framework from the key's *deterministic home region* (the same key-hash → region → country the REST route uses — `eu-west-1→DE→GDPR`, `ap-south-1→IN→DPDP`, else NONE), runs `enforceOptOutPropagation` then `applyPrivacyMasking` for live keys (sandbox unmasked), and reports whether redaction actually applied. Returns `{ data, errors?, extensions:{ cost, remaining, masked, framework, optOutsRemoved, environment, requestId } }` with `X-GraphQL-Cost` + `X-Privacy-Framework`. `401` for a bad key, `400` for a bad body, `402` on credits. The same `sk_live_` key masks identically via GraphQL and REST — one governed surface, not a side door.
