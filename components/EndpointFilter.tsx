  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className="flex items-center gap-3 bg-surface-2/80 backdrop-blur-md border border-border hover:border-teal/50 hover:bg-glass rounded-xl px-4 py-2 transition-all shadow-xl group"
      >
        <div 
          className="w-2.5 h-2.5 rounded-full" 
          style={{ backgroundColor: selectedEndpoint.color, boxShadow: `0 0 10px ${selectedEndpoint.color}80` }} 
        />
        <span className="text-sm font-bold text-fg group-hover:text-teal transition-colors">
          {selectedEndpoint.label}
        </span>
        <ChevronDown className={cn("w-4 h-4 text-fg-muted transition-transform duration-300", isOpen && "rotate-180 text-teal")} />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-2 w-64 bg-surface-2 border border-border rounded-2xl shadow-[0_20px_40px_-10px_rgba(0,0,0,0.7)] overflow-hidden z-50 p-2"
          >
            <div className="mb-2 px-3 pt-2 pb-1 text-xs font-bold text-fg-muted uppercase tracking-widest border-b border-border-subtle flex items-center gap-2">
