...
export function PricingSliderModal({ isOpen, onClose }: PricingSliderModalProps) {
  ...
  const getLevel = () => {
    ...
    return { name: 'Starter', discount: 0, color: 'text-fg' };
  };
        ...
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-2xl bg-surface border border-border rounded-3xl shadow-2xl overflow-hidden z-10 flex flex-col"
            >
              {/* Header */}
              <div className="p-8 pb-6 border-b border-border-subtle relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-teal/10 blur-[100px] rounded-full pointer-events-none" />
                <div className="relative z-10">
                  <h2 className="text-3xl font-display font-black text-fg mb-2">Build Your Plan</h2>
                  <p className="text-fg-muted">Drag the slider to set your API volume. Higher volume unlocks steeper discounts.</p>
                </div>
                <button onClick={onClose} className="absolute top-6 right-6 p-2 text-fg-muted hover:text-fg transition-colors rounded-lg hover:bg-glass z-10">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-10 space-y-8 bg-gradient-to-b from-white/[0.02] to-transparent">
                ...
                <div className="flex justify-center">
                  <motion.div 
                    ...
                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border border-border bg-glass font-bold ${level.color}`}
                  >
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <span className="text-sm font-bold text-fg-muted uppercase tracking-widest">Monthly API Calls</span>
                    <span className="text-3xl font-black text-fg font-mono">{volume.toLocaleString()}</span>
                  </div>
                ...
                {/* Price Calculation */}
                <div className="glass-panel p-6 rounded-2xl border border-border-subtle space-y-4 relative overflow-hidden">
                  {level.discount > 0 && (
                  ...
                  <div className="flex justify-between items-end pt-4 border-t border-border-subtle">
                    <span className="font-bold text-fg">Your Price</span>
                    <div className="text-right">
                      <span className="text-4xl font-black text-fg">${finalPrice}</span>
                      <span className="text-fg-muted ml-1">/mo</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Footer */}
              <div className="p-8 border-t border-border-subtle flex flex-col sm:flex-row items-center justify-between gap-6 bg-surface">
