...
import { LogOut, MessageSquare, Menu, X, ChevronDown, Check, Plus, Building2, ShieldAlert, Rocket } from 'lucide-react';
...
export default function Phase1ConsoleLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    track('feature_viewed', { feature: pathname.replace(/^\/phase-1\/console\/?/, '') || 'command-center' });
  }, [pathname]);
  ...
  const activeSectionId = visibleSections.find((s) =>
    s.items.some((i) => pathname === i.href || (pathname.startsWith(i.href + '/') && i.href !== '/phase-1/console/overview')),
  )?.id ?? null;
  ...
  const currentNavItem = allNavItems.find(item => pathname === item.href || (pathname.startsWith(item.href + '/') && item.href !== '/phase-1/console' && item.href !== '/phase-1/console/overview'));
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-surface relative w-full">
        ...
        <header className="h-16 bg-surface/60 backdrop-blur-xl border-b border-border flex items-center justify-between px-4 md:px-8 flex-shrink-0 z-20">
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/phase-1"
              title="Phase-1 launch scope — back to Launch Center"
              className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest text-teal bg-teal/10 hover:bg-teal/20 border border-teal/30 rounded-full transition-colors"
            >
              <Rocket className="w-3 h-3" /> Phase 1
            </Link>
          </div>
          ...
          <div className="flex items-center space-x-3 md:space-x-6">
            ...
            <a
              href="/phase-1/admin/overview"
              title="Open Zinbit Admin — Phase-1 operator scope"
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10 w-full overflow-x-hidden">
          {!isAuthorized ? (
            ...
            <ProtectedRoute allowedRoles={allNavItems.find(item => pathname.startsWith(item.href) && item.href !== '/phase-1/console')?.roles as ('admin' | 'developer' | 'billing')[] | undefined || ['admin', 'developer', 'billing']}>
              <GrowthAlertsWatcher />
              <NudgeOrchestrator />
              <NudgeWatcher />
