...
export default function BillingSettingsPage() {
  const { billingDetails, updateBillingDetails, pocContacts, addPOCContact, removePOCContact } = useStore();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12 animate-in fade-in duration-500">
      ...
      {/* Header */}
      <div>
        <Link href="/console/billing" className="inline-flex items-center gap-2 text-sm font-bold text-fg-muted hover:text-fg transition-colors mb-4">
          <ArrowLeft className="w-4 h-4" /> Back to Billing
        </Link>
        <h1 className="text-3xl font-display font-extrabold text-fg mb-2 tracking-tight">Billing Settings & Contacts</h1>
        <p className="text-fg-muted font-medium">Manage your company legal information and assign roles for notifications.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-2xl border border-border space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-fg-muted uppercase tracking-wider mb-1.5">Legal Company Name</label>
              <input 
                type="text" 
                value={details.companyName}
                onChange={e => setDetails({...details, companyName: e.target.value})}
                className="w-full bg-surface-2 border border-border rounded-xl px-4 py-3 text-fg focus:outline-none focus:border-teal/50 transition-colors"
                placeholder="e.g. Acme Corporation"
              />
            </div>
            
            <div>
              <label className="block text-xs font-bold text-fg-muted uppercase tracking-wider mb-1.5">Tax ID / VAT Number</label>
              <input 
                type="text" 
                value={details.taxId}
                onChange={e => setDetails({...details, taxId: e.target.value})}
                className="w-full bg-surface-2 border border-border rounded-xl px-4 py-3 text-fg focus:outline-none focus:border-teal/50 transition-colors font-mono"
                placeholder="e.g. US123456789"
              />
            </div>
            
            <div>
              <label className="block text-xs font-bold text-fg-muted uppercase tracking-wider mb-1.5">Billing Address</label>
              <textarea 
                value={details.address}
                onChange={e => setDetails({...details, address: e.target.value})}
                className="w-full bg-surface-2 border border-border rounded-xl px-4 py-3 text-fg focus:outline-none focus:border-teal/50 transition-colors h-24 resize-none"
        ...
        {/* POC Manager Section */}
        <div className="glass-panel p-6 sm:p-8 rounded-2xl border border-border flex flex-col h-full">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-fg flex items-center gap-2 mb-1">
                <User className="w-5 h-5 text-teal" />
                Points of Contact
              </h2>
              <p className="text-xs text-fg-muted font-medium">Route alerts to the right teams.</p>
            </div>
            {!isAddingPOC && (
              <button 
          ...
          <div className="flex-1 space-y-3 overflow-y-auto">
            {isAddingPOC && (
              <div className="p-4 bg-surface-2 border border-teal/30 rounded-xl space-y-3 mb-4 shadow-[0_0_15px_rgba(70,189,198,0.1)]">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-bold text-teal uppercase tracking-wider">New Contact</span>
                  <button onClick={() => setIsAddingPOC(false)} className="text-fg-muted hover:text-fg text-xs">Cancel</button>
                </div>
                ...
                <input 
                  ...
                  className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-fg focus:outline-none focus:border-teal/50"
                />
                <input 
                  type="email" 
                ...
                <select 
                  value={newPOC.role}
                  onChange={e => setNewPOC({...newPOC, role: e.target.value as POCContact['role']})}
                  className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-fg focus:outline-none focus:border-teal/50"
                >
            ...
            {pocContacts.map((contact) => (
              <div key={contact.id} className="p-4 bg-surface-2 border border-border-subtle rounded-xl group hover:border-border transition-colors flex flex-col gap-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-glass flex items-center justify-center font-bold text-fg border border-border">
                      {contact.name.charAt(0)}
                    </div>
                    <div>
                      <h4 className="text-fg font-bold text-sm leading-none mb-1">{contact.name}</h4>
                  ...
                  <button 
                    ...
                    className="p-1.5 text-fg-subtle hover:text-semantic-error hover:bg-semantic-error/10 rounded-md transition-all opacity-0 group-hover:opacity-100"
                  >
            ...
            {pocContacts.length === 0 && !isAddingPOC && (
              <div className="text-center py-8 text-fg-muted border border-dashed border-border rounded-xl">
