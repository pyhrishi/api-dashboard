...
export const BUILT_FEATURE_IDS: ReadonlySet<string> = new Set([
  ...
  'F-312', // Encryption in transit & at rest — lib/encryption.ts SSOT (deterministic TLS posture + KMS key inventory + envelope-encrypted store catalog + field-level PII modes + postureScore/attest + dedicated persisted useEncryptionSettings store) wired to a REAL gateway meta module src/lib/gateway/encryption.ts (GET/POST /v1/encryption live signed attestation + rotation drill + X-Encryption-Transit/X-Encryption-Rest headers on EVERY response) + catalog entry + /console/encryption (transit inspector w/ live header check, KMS rotation, encrypted-store table, PII field toggles, copyable attestation)
