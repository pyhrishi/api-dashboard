...
export const BUILT_FEATURE_IDS: ReadonlySet<string> = new Set([
  ...
  'F-011', // Email deliverability scoring — Studio "Verify deliverability" (GET /v1/email/verify)
  ...
  'F-131', // Tier-based throughput — lib/throughput-tiers.ts SSOT (TIER_LIMITS Starter/Growth/Enterprise + deterministic tierForKey, Edge-safe) wired into rateLimiter.ts so a key's plan sizes its bucket (burst + sustained) → RateLimit-Limit + X-RateLimit-Tier vary by tier (F-130 headers advertise it) + /console/throughput-tiers ladder + upgrade CTA
  'F-134', // Graceful 429 with retry-after — lib/retry-after.ts SSOT (parseRetryAfter seconds|HTTP-date, deterministic seeded-jitter backoffDelay honouring Retry-After capped, isRetryableStatus 429/5xx, retryTimeline + fetchWithRetry with injectable sleep/fetch) + /console/retry-strategy (two backoff ladders, a REAL live 429 trigger draining a fresh key + reading its Retry-After, copyable fetchWithRetry snippet) — the client-side companion to F-129/F-130
  'F-199', // Trace ID on every request — X-Request-Id set + returned on every request
  'F-303', // Web application firewall — lib/waf-rules.ts SSOT (rule catalog: SQLi/command-injection/XSS/path-traversal/file-inclusion signatures + severity + deterministic inspectString/inspectRequest) consumed by BOTH src/lib/gateway/waf.ts (real edge 406 block, severity in threatLevel, bug-bounty safe-harbor bypass preserved) AND /console/waf (rule catalog, client payload tester, REAL live gateway probe reading the actual 406) — advertised rules == enforced rules
  'F-306', // Brute-force login protection — lib/brute-force.ts (progressive lockout SSOT + dedicated persisted useLoginGuard store, pre-auth) ENFORCED in app/login/page.tsx (locks after N fails, escalating cooldown 30s→2m→15m→1h, resets on success) + /console/login-security incident console (locked accounts, policy, live simulator, admin unlock)
  'F-309', // MFA enforcement — lib/mfa.ts (org MFA policy + per-member enrollment in a dedicated persisted useMfaPolicy store + deterministic TOTP secret/recovery codes + orgCompliance/enrollmentRequired helpers) ENFORCED via MfaEnforcementBanner (console-wide gate) + login redirect + /console/mfa (admin required/optional policy, TOTP enrollment, member compliance roster)
  'F-312', // Encryption in transit & at rest — lib/encryption.ts SSOT (deterministic TLS posture + KMS key inventory + envelope-encrypted store catalog + field-level PII modes + postureScore/attest + dedicated persisted useEncryptionSettings store) wired to a REAL gateway meta module src/lib/gateway/encryption.ts (GET /v1/encryption HMAC-SHA256-signed attestation + PATCH settings sync so console and gateway agree + POST rotation drill + GET /v1/encryption/verify + X-Encryption-Transit/X-Encryption-Rest headers on EVERY response, CORS-exposed; HSTS derived from the same SSOT) + catalog entry + /console/encryption (transit inspector w/ live header check + error state, KMS rotation, encrypted-store table, PII field toggles with the randomized floor enforced in the SSOT, copyable signed attestation) + Security Hub posture card + Reset Demo Data clears the security stores
  'F-315', // Content-security policy — lib/csp.ts pure Edge-safe SSOT (directive catalog + buildCspHeader nonce-aware + Report-To/hardening headers + generateNonce + parseViolationReport for both csp-report + Reporting-API shapes) DELIVERED by middleware.ts on every page nav (Content-Security-Policy-Report-Only by default, enforce via a cookie the Edge middleware reads → toggle actually changes the served header; /api/v1 branch untouched) + src/lib/gateway/cspReports.ts collector + POST/GET /api/csp-report (a standalone keyless route, NOT the gateway pipeline) + /console/csp (mode toggle, rendered policy + per-directive cards, trigger-test-violation, live violation feed w/ by-directive + top-blocked aggregation)
  'F-314', // Payload size & depth limits — lib/payload-limits.ts SSOT (six dimensions: body bytes / JSON depth / array length / total keys / string length / URL length; per-tier ceilings off the throughput-tier ladder; iterative stack-safe measureJson; evaluatePayload → typed violations with a concrete fix + chunk plan; tighten-only org overrides in a dedicated persisted usePayloadLimits store) ENFORCED by src/lib/gateway/payloadLimits.ts on EVERY non-GET body before any sub-router (413 PAYLOAD_TOO_LARGE / 414 URI_TOO_LONG / 422 PAYLOAD_TOO_DEEP|ARRAY_TOO_LONG|TOO_MANY_KEYS|STRING_TOO_LONG with details.fix) + X-Payload-Limit-Bytes/Depth on every response (CORS-exposed) + GET/PATCH /v1/limits/payload + free POST /v1/limits/payload/check dry run + rejection ledger; catalog entry; insight-engine explains 413/414/422; /console/payload-limits (limits editor, payload analyzer w/ deterministic presets + per-dimension meters + chunk plan, real gateway dry run + send-for-real, rejection ledger, upgrade nudge)
  'F-321', // API keys hashed at rest — lib/key-hashing.ts SSOT (pure sync SHA-256 that runs in the browser, the Edge middleware and Node; hashApiKey/keyFingerprint sha256:<16hex>/redactKey/constantTimeEqual/toKeyAtRest with NO plaintext field by type; KEY_HASHING_POSTURE) — EVERY gateway registry that references a key is keyed by the digest (billing + IP allowlist already were; converted: scope registry, kill switch, rate-limit buckets on Edge, idempotency, geo-velocity tracker, partner attribution; masked display captured at write time) + orgHandleForKey and the F-115 reveal fingerprint now derive from the same digest (no plaintext fragments at rest) + src/lib/gateway/keyHashing.ts (auditKeyStorage → registries keyed-by + plaintextCopies=0, describeKeyAtRest, constant-time verifyHash accepting digest/fingerprint only) + GET /v1/keys/hashing + POST /v1/keys/hashing/verify + X-Key-Fingerprint on every response (CORS-exposed) + catalog entry + fingerprint chip on /console/keys + /console/key-hashing (keys-at-rest table, browser-local hash inspector w/ digest-only gateway verify, live registry attestation w/ console↔gateway match badge, storage policy)
  'F-310', // Session management — lib/sessions.ts SSOT (deterministic risk scoring: new-country/new-city/external-network/idle-past-policy/impossible-travel with plain-English reasons + parseDevice + summarizeSessions + staleSessionIds, no Math.random) on an extended store (ActiveSession +createdAt/+type, sessionPolicy persist-v5 migrate, revokeAllOtherSessions/revokeSessions/updateSessionPolicy keeping the current session invariant) + /console/sessions (risk callout, this-device + others, per-session revoke, revoke-all-stale, sign-out-everywhere, admin policy panel) cross-linked from settings/security
  ...
  'F-033', // Merge & unmerge controls — /console/identity merge center (lib/merge-seed.ts + store slice) — confirm/split entity resolution, audited & reversible
  ...
  'F-056', // Currency normalization — lib/currency-normalizer.ts (parse symbols/ISO codes/scale words incl. lakh+crore + US/EU/FR/Indian/Swiss locale grouping → ISO 4217 + canonical amount + frozen-FX conversion to target, ambiguity notes) + GET /v1/currency/normalize + Studio preset
  ...
  'F-322', // PII redaction in internal logs — lib/log-redaction.ts SSOT (pure Edge/browser-safe engine: 10-type catalog = F-313 PII_CATALOG + payment card + secrets; key-based classification via classifyKey + secret key patterns + org customKeys/allowKeys, value-based detectors for email/phone(E.164+formatted, date-guarded)/IPv4/SSN-Aadhaar-PAN/Luhn+IIN cards/LinkedIn-X URLs/sk_ keys-Bearer-JWT; strategies partial<token<drop with un-relaxable floors, NO master switch; deterministic per-org correlation tokens tk_<12hex> = sha256(orgSalt:type:value); secrets → F-321 sha256: fingerprint; redactLogRecord/redactString/selfTest on CANARY_RECORD/normalizeLogPolicy+Patch/logPolicyStrength/LOG_REDACTION_POSTURE + persisted useLogRedactionPolicy store) — the gateway's ONLY path to stdout: src/lib/gateway/logRedaction.ts writeLine (redact-before-serialize, per-org metrics + 40-line tail of lines exactly as written, dryRunRedaction, report w/ self-test) behind logger.ts Logger/logRequest (route.ts attributes lines by requestId + apiKey → fingerprint) + GET /v1/logs/redaction + PATCH (floor-clamped sync, reports ignored parts) + POST /v1/logs/redaction/test (dry run, nothing logged) + X-Log-Redaction: <n> on every response (CORS-exposed) + catalog entry + lib/redaction-engine.ts value detection delegates to the SSOT + /console/log-redaction (KPIs, Redaction Tester w/ before/after + findings + gateway dry-run match badge, live internal log tail w/ send-sample + request-id → Logs, admin policy editor w/ floors + custom/allow keys + retention + sync badge, canary attestation table + copy-for-reviewer, posture) + Security Hub card + Logs privacy drawer link + Reset Demo Data clears the store
  'F-081', // Dark-launch preview endpoints — lib/preview-program.ts SSOT (preview catalog w/ alpha/beta/preview stages, whatsNew, target GA) + lib/preview-resolvers.ts (same resolvers as GA where equivalent + deterministic buying-signals alpha) + real gated GET /api/preview[/<id>] (x-preview-optin gate → 403 PREVIEW_ACCESS_REQUIRED, free during dark launch) + tenant-scoped previewOptIns store slice (enroll/leave, RBAC, audit) + /console/preview enroll + live 403↔200 try-it
  'F-387', // Progress-based nudges — SUPERSEDED & shipped as the full lifecycle-nudge system: lib/lifecycle.ts (C0-a…C7-b state machine + triggers + lead score) + lib/nudges.ts (catalog/selector/useNudgeState) + lib/nudge-delivery.ts (re-engagement ledger) + components/nudges/* (orchestrator/banner/modal/celebration/watcher, layout-mounted) + /console/journey cockpit + pre-auth TOFU on /signup; Mixpanel adapter + full event taxonomy in lib/telemetry.ts
  'F-503', // Free tier / trial — risk-based phone OTP at trial activation (shared-auth prototype): lib/auth/trial-gate.ts SSOT (six conditions C1 non-ICP / C2 low domain reputation / C3 domain unmapped / C4 small company / C5 duplicate domain / C6 duplicate IP; C2+C5 locked on; per-product policy w/ clamped thresholds, SMS-degraded countries, shared-egress allow-list, referral waives C4; evaluateTrialRisk reuses company/disposable/domain-auth resolvers, deterministic domain age; payment/enterprise/phone exemptions; seeded cross-product directory + 6 simulator personas) + lib/auth/phone-otp.ts (E.164 normalize w/ 16-country table, country from IP, temp-phone verifier: repeated/sequential digits, US 555-01xx + 5xx VoIP, UK 07700 900 + 070/056, IN 99999 patterns, landline→voice only, reuse + denylist; 6-digit OTP from server secret, hashed, 10-min TTL, 5 attempts, 3 sends/hour, 3 rejected numbers → 24h lock; channels SMS→WhatsApp→voice w/ degraded-country primary + documented delivery simulator) + src/lib/auth/trial-gate-service.ts (per-product policy, directory, evaluations, challenges, sends, rejections, overrides w/ mandatory reason, verified-phone 180-day reuse index, 200-event stream, sandbox inbox, stats) + public /api/auth/* routes (trial/evaluate, phone/challenge|resend|verify|abandon|inspect|override, policy GET/PATCH, challenges/overrides/rejections/events/directory/stats/personas/countries, demo/inbox) + store trialActivation slice (persisted; reset on signup) + components/TrialActivationGate.tsx (console banner → evaluate → instant activation or why-flagged → phone → code w/ 30s fallback timer, WhatsApp/call, sandbox phone, lock, resume after close/reload; grants 10,000 credits) + phone-free /signup copy + /console/trial-gate T&S console (KPIs, persona simulator, number checker, policy editor w/ locks, challenge ledger w/ allow/deny overrides, delivery & fallback stats, events)
  // ── PLG KPI framework (2026-09-08): the measurement layer behind /console/growth ──
  'F-390', // Time-to-first-call tracking — lib/growth-kpis.ts SSOT (TOFU/MOFU/BOFU funnel with drop-off % per stage; durationStats min/max/avg/median/p90 + share within the <10 min ACTIVATION_TARGET; time-to-first-key; liveDeveloperRecord reconstructs this console's own funnel from real events, and the Explorer now emits first_call_made on the first successful Run so activation is never under-counted) + a deterministic seeded developer cohort (COHORT_SIZE, anchored to the day, labelled as a sample) merged with the live workspace so medians / bands / week-over-week math have substance + 4 alert rules with owners (activation WoW < −10 pp → Product · OTP completion < 60% → Product [auth-service source] · top-up failure > 5% → Eng · docs no-results > 20% → Docs owner) evaluated from the same data + deterministic scenarios that make them fire + copyable weekly PM report (Markdown) — surfaced on /console/growth; new fire points: destructive_action_confirmed (ConfirmAction primitive), credits_recharge_failed (a real declined-card path in RechargeModal), docs_search_performed (DocsSearch on /docs)
  'F-195', // Endpoint popularity heatmap — endpointHeatmap() in lib/growth-kpis.ts: endpoint × UTC-hour utilization from the real request log (exact) and the cohort's regional business-hours mix, top-12 endpoints, peak hour — rendered on /console/growth
  'F-530', // Cohort analysis — revenueCohorts() by signup month (developers, activated, paid, paid %, revenue, revenue per developer), usageBands() power-user 10% bands by 7-day calls, engagement() DAU/WAU 28-day series + stickiness, healthMetrics() tickets per active dev + destructive-action incident rate — all on /console/growth, scope-switchable between your workspace and the sample cohort
  'F-194', // Custom metric alerts — lib/growth-alerts.ts (F-194/F-526/F-553 SSOT): per-rule threshold overrides clamped to THRESHOLD_BOUNDS (tighten or relax, never off) feeding evaluateAlerts via effectiveRules; incidents open per rule/ISO-week/org, acknowledge with note, manual or auto resolve on recovery, rehearsal incidents from Growth scenarios tagged and isolated; persisted useAlertCenter store with admin-only config + role-gated ack; components/GrowthAlertsWatcher.tsx evaluates in the console layout so alerts reach someone without the Growth page open; components/NotificationBell.tsx header bell (unseen count, ack from the bell); nav badge + Overview "Growth pulse" strip + per-org open-alert count + ⌘K actions; /console/alerts Alert Center (incidents, rules & thresholds, routing per owner, delivery ledger)
  'F-553', // Multi-channel delivery — routing per alert owner across in-app (floor, always on) / email / Slack / webhook with honest deterministic outcomes (no recipient → failed, bad channel → failed), a delivery ledger per attempt, "send test alert" per owner — lib/growth-alerts.ts planDeliveries/resolveTargets, surfaced on /console/alerts
  'F-526', // Scheduled report delivery — the weekly PM digest: cadence (weekly weekday / daily, UTC hour), recipients, channels, scope; nextDigestRunAt/digestIsDue scheduler run by GrowthAlertsWatcher when due (recorded at the scheduled point), Send-now, full Markdown history with viewer + copy — lib/growth-alerts.ts + /console/alerts#digest
  'F-575', // Super-admin account impersonation — built in the separate zinbit-admin app (Preview as customer: ops/superadmin mint a 30-min sandbox sk_test_ token, returned once, audit-logged; the console consumes it at /console/preview-session + components/ImpersonationBanner.tsx, adds a 'Preview · <customer>' sandbox key and labels every screen; the old masked-key bug is rejected by shape) — spec docs/prd/admin-panel-b2b2b-operations.md
  'F-521', // Automated AE handoff triggers — built in the separate zinbit-admin app (stage/metric rules with thresholds, channels, cooldown → handoff records with evidence and new→contacted→converted/closed states; evaluated at boot and on demand; sales-role edits audited)
]);
